...
func resourceKsyunMysqlUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if execModifyDBInstance {
		req := map[string]interface{}{
			...
			"DBInstanceNterame":    d.Get("db_instance_name"),
			"MasterUserPassword":   d.Get("master_user_password"),
			"SecurityGroupId":      d.Get("security_group_id"),
			//"DBParameterGroupId":   d.Get("db_parameter_group_id"),
		}
	...
	if execModifyDBBackupPolicy {
		...
		logger.Debug(logger.ReqFormat, action, req)
		resp, err := conn.ModifyDBBackupPolicy(&req)
		logger.Debug(logger.AllFormat, action, req, *resp, err)
		if err != nil {
			return err
		}
	...
	if execModifyDBInstanceAvailabilityZone {
		...
		req := map[string]interface{}{
			"DBInstanceIdentifier": d.Id(),
			"AvailabilityZone.1":   d.Get("availability_zone.1"),
			"AvailabilityZone.2":   d.Get("availability_zone.2"),
		}
		...
		if d.HasChange("availability_zone.1") {
			d.SetPartial("availability_zone.1")
		}
		if d.HasChange("availability_zone.2") {
			d.SetPartial("availability_zone.2")
		}
	}
...
func checkStatus(d *schema.ResourceData, conn *krds.Krds) bool {
	stateConf := &resource.StateChangeConf{
		Pending:    waitStatus,
		Target:     finalStatus,
	...
	if err != nil {
		fmt.Errorf("error on updating ModifyDBInstanceType , err = %s", err)
		return false
	} else {
		_, instFinalStatus, _ := mysqlInstanceStateRefresh(conn, d.Id(), finalStatus)()

		//instFinalStatus := mysqlInstanceStateRefresh(conn, d.Id())
		if instFinalStatus != tActiveStatus {
			fmt.Errorf("error status : %s, ", instFinalStatus)
			return false
		} else {
			return true
		}
